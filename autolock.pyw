...
# is autolocked
#
# A quick hack by Davide Nastri, July 2017
...
# Program logic
while(True):
    if locker.is_user_present():
        print "User is active, all is good!"
        locker.reset_seconds_user_missing_counter()
    else:
        print "User is not active since " + str(locker.seconds_user_missing_counter) + " seconds..."
        if locker.is_user_presence_timeout_reached():
